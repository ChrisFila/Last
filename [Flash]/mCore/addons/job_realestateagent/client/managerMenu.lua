...
local cat, desc, servInteract = "realestateagentmanager", "~r~Interaction avec la propriétée", false
...
FlashSide.netRegisterAndHandle("houseDepositFailed", function(callbackMessage)
    servInteract = false
    info.type = 2
    info.message = callbackMessage
end)

FlashSide.netRegisterAndHandle("houseDepositSucceed", function(itemName, newCount, depositCout)
...
FlashSide.netRegisterAndHandle("houseRemovalSucceed", function(itemName, newCount, removalCount)
...
FlashSide.netRegisterAndHandle("openManagerPropertyMenu", function(onlinePlayers, allowedPlayers, license, houseId, isPublic, inventories, capacity)
    info.type = nil
    ...
    local function depositAbility(have, wantToDeposit)
        if have < wantToDeposit then
            ...
            return { ret = true, display = ("Déposer x~r~%i ~s~→→"):format(wantToDeposit) }
        end
    end
    ...
    local function removalAbility(have, wantToDeposit)
        if have < wantToDeposit then
            ...
            return { ret = true, display = ("Retirer x~r~%i ~s~→→"):format(wantToDeposit) }
        end
    end
    ...
    FlashSide.newThread(function()
        while menuIsOpened do
            ...
            RageUI.IsVisible(RMenu:Get(cat, sub("main")), true, true, true, function()
                tick()
                RageUI.Separator("↓ ~r~Gestion de la propriétée ~s~↓")
            ...
            RageUI.IsVisible(RMenu:Get(cat, sub("safe")), true, true, true, function()
                ...
                RageUI.Separator("↓ ~r~Gestion du coffre fort ~s~↓")
                ...
                RageUI.Separator("↓ ~r~Objets disponibles ~s~↓")
                ...
                for itemName, itemInfos in pairs(propertyInv) do
                    if itemInfos.count > 0 then
                        RageUI.ButtonWithStyle(("%s ~s~(~r~%i~s~)"):format(itemInfos.label, itemInfos.count), nil, { RightLabel = removalAbility(itemInfos.count, depositIndex).display }, not servInteract, function(_, _, s)
                            if s then
                                if not depositAbility(itemInfos.count, depositIndex).ret then
                                    ...
                                    if not servInteract then
                                        ...
                                        FlashSideClientUtils.toServer("removeFromHouse", houseId, itemName, depositIndex)
                                    end
                                end
                            end
                        end)
            ...
            RageUI.IsVisible(RMenu:Get(cat, sub("safe_deposit")), true, true, true, function()
                ...
                RageUI.Separator(("Capacité: ~o~%i~s~/~o~%i"):format(countItems(propertyInv), capacity))
                RageUI.Separator("↓ ~r~Déposer des objets ~s~↓")
                RageUI.List("Quantité à déposer:", depositList, depositIndex, nil, {}, true, function(_, _, _, i)
                    depositIndex = i
                end)
                RageUI.Separator("↓ ~r~Vos objets ~s~↓")
                for itemName, itemInfos in pairs(playerInv) do
                    if itemInfos.count > 0 then
                        RageUI.ButtonWithStyle(("%s ~s~(~r~%i~s~)"):format(itemInfos.label, itemInfos.count), nil, { RightLabel = depositAbility(itemInfos.count, depositIndex).display }, not servInteract, function(_, _, s)
                            if s then
                                if not depositAbility(itemInfos.count, depositIndex).ret then
                                    info.type = 2
                                    ...
                                    if not servInteract then
                                        ...
                                        info.type = 1
                                        info.message = "Transaction avec le serveur en cours..."
                                        FlashSideClientUtils.toServer("depositHouse", houseId, itemName, depositIndex)
                                    end
                                end
                            end
                        end)
            ...
            RageUI.IsVisible(RMenu:Get(cat, sub("invite")), true, true, true, function()
                tick()
                RageUI.Separator("↓ ~r~Gestion des autorisations ~s~↓")
                RageUI.ButtonWithStyle("Appliquer les autorisations", nil, { RightLabel = "→→" }, true, function(_, _, s)
                    if s then
                        ...
                        FlashSideClientUtils.toServer("setHouseAlloweds", houseId, autorisationTable, isPublic)
                ...
                if not isPublic then
                    RageUI.Separator("↓ ~r~Autorisations ~s~↓")
                    for k, v in pairs(onlinePlayers) do
                        RageUI.Checkbox(("~o~%s ~s~→ ~r~%s"):format(k, v.name), nil, autorisationTable[v.license].can, { Style = RageUI.CheckboxStyle.Tick }, function(Hovered, Selected, Active, Checked)
