...
AddEventHandler('fUnicorn:Ouvert', function()
  ...
  if xPlayer.job.name == 'unicorn' then
    for i=1, #xPlayers, 1 do
      ...
      TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Vanilla Unicorn', '~r~Informations', 'L\'Unicorn est ouvert', 'CHAR_MP_STRIPCLUB_PR', 2)
    end
  end
end)
...
AddEventHandler('fUnicorn:Fermer', function()
  ...
  if xPlayer.job.name == 'unicorn' then
    for i=1, #xPlayers, 1 do
      local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
      TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Vanilla Unicorn', '~r~Informations', 'L\'Unicorn est fermé', 'CHAR_MP_STRIPCLUB_PR', 2)
    end
  end
end)
...
AddEventHandler('fUnicorn:Perso', function(msg)
    ...
    if xPlayer.job.name == 'unicorn' then
      for i=1, #xPlayers, 1 do
          local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
          TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Vanilla Unicorn', '~r~Annonce', msg, 'CHAR_MP_STRIPCLUB_PR', 8)
      end
    end
end)
