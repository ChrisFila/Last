...
local function Display(ped, text)
    ...
    if dist <= distToDraw then
        ...
        local display = true
        FlashSide.newWaitingThread(displayTime, function()
            display = false
        end)
...
RegisterCommand('me', function(source, args)
    if allowed then
        ...
        FlashSideClientUtils.toServer("shareDisplay", idsToSend, text, pPedSID)
        ActionMeCoolDown()
    end
end)
...
FlashSide.netRegisterAndHandle('shareDisplay', function(text, serverId)
...
function ActionMeCoolDown()
    allowed = false
    FlashSide.newWaitingThread(6*1000, function()
        allowed = true
    end)
end

FlashSide.netRegisterAndHandle('adminTag:trigger', function(sender, message)
