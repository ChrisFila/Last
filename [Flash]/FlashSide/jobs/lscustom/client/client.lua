     ...
     if open then 
         ...
         Citizen.CreateThread(function()
			...
			if IsPedSittingInAnyVehicle(playerPed) then
				ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Vous ne pouvez pas faire cette action depuis un véhicule", 'CHAR_CARSITE3', 7)
				return
			end
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Réparation ~h~~r~Terminer", 'CHAR_CARSITE3', 7)
					isBusy = false
				end)
			else
				ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Aucun véhicule à proximité", 'CHAR_CARSITE3', 7)
			end
                    end
                   })
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Nettoyage ~h~~r~Terminer ", 'CHAR_CARSITE3', 7)
					isBusy = false
				end)
			else
				ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Aucun véhicule à proximité", 'CHAR_CARSITE3', 7)
			...
			if IsPedSittingInAnyVehicle(playerPed) then
                ...
                Citizen.Wait(2500)
                ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Mise en fourrière ~h~~r~Terminer", 'CHAR_CARSITE3', 7)
                ESX.Game.DeleteVehicle(vehicle)
				else
					ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Vous devez être sur la place conducteur !", 'CHAR_CARSITE3', 7)
				end
			else
				local vehicle = ESX.Game.GetVehicleInDirection()
				...
				if DoesEntityExist(vehicle) then
					...
					ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Vous devez être prêt d'un véhicule pour commencer la mise en fourrière", 'CHAR_CARSITE3', 7)
				end
			end
                    end 
                })
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Crochetage ~h~~r~Terminer", 'CHAR_CARSITE3', 7)
					isBusy = false
				end)
			else
				ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Aucun véhicule à proximité", 'CHAR_CARSITE3', 7)
						...
						if targetVehicle ~= 0 then
							if not IsPedInAnyVehicle(playerPed, true) then
								if vehicle ~= targetVehicle then
									...
									ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Vous venez d'attacher le véhicule avec ~h~~r~succés !", 'CHAR_CARSITE3', 7)
									...
									ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Vous ne pouvez pas attacher votre ~h~~r~propre véhicule de dépannage !", 'CHAR_CARSITE3', 7)
								end
							end
						else
							ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Il n'y a pas de véhicule ~h~~r~attacher !", 'CHAR_CARSITE3', 7)
						end
					else
						...
						if NPCOnJob then
							if NPCTargetDeleterZone then
								...
								if CurrentlyTowedVehicle == NPCTargetTowable then
									...
									ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Ce n'est pas le bon ~h~~r~véhicule !", 'CHAR_CARSITE3', 7)
								end
	
							else
								ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Vous n'etes pas au bon endroit ~h~~r~pour faire cela !", 'CHAR_CARSITE3', 7)
							end
						end
	
						CurrentlyTowedVehicle = nil
						ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Véhicule détacher avec ~r~~h~succès !", 'CHAR_CARSITE3', 7)
					end
				else
					ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Vous devez avoir une ~r~~h~dépanneuse à plateau pour faire cela !", 'CHAR_CARSITE3', 7)
				end
                    end
                })
            end)
                ...
                RageUI.IsVisible(mainMenuFrabrication,function() 
                  ...
                  RageUI.Button("Fabriquer un kit de réparation", 'Prix du matériel néceissaire : ~r~10$', {RightLabel = "→→"}, true , {
                    onSelected = function()
                        ...
                        TaskStartScenarioInPlace(playerPed, 'PROP_HUMAN_BUM_BIN', 0, true)
                        Citizen.CreateThread(function()
                        ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Fabrication ~r~~h~en cours...", 'CHAR_CARSITE3', 7)
                        Citizen.Wait(7500)
                        ClearPedTasksImmediately(playerPed)
                        Citizen.Wait(100)
                ...
                RageUI.IsVisible(mainMenuvestiaire,function() 
                  ...
                  RageUI.Checkbox("Prendre sa tenue LSCustom", nil, service, {}, {
                    onChecked = function(index, items)
                        service = true
                        serviceon()
                        ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Vous venez de prendre votre ~r~tenue", 'CHAR_CARSITE3', 7)
                    end,
                    onUnChecked = function(index, items)
                        service = false
                        serviceoff()
                        ESX.ShowAdvancedNotification('lscustom', '~r~Notification', "Vous venez de ~r~ranger votre tenue", 'CHAR_CARSITE3', 7)
                    end
                })
