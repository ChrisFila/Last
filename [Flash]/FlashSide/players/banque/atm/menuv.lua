    ...
    if zPlayer ~= nil and GetPlayerEndpoint(to) ~= nil then
        ...
        if tonumber(_source) == tonumber(to) then
            ...
            if balance <= 0 or balance < tonumber(amountt) or tonumber(amountt) <=
                ...
                TriggerClientEvent('esx:showAdvancedNotification', _source,'Banque', 'Transférer de l\'argent','Vous avez transféré ~r~$' .. amountt ..'~s~ à ~r~' .. to .. ' .','CHAR_BANK_MAZE', 9)
                TriggerClientEvent('esx:showAdvancedNotification', to, 'Banque','Transférer de l\'argent', 'Vous avez reçu ~r~$' ..amountt .. '~s~ de ~r~' .. _source ..' .', 'CHAR_BANK_MAZE', 9)
            end
        end
    end
end)
