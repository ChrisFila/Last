...
Citizen.CreateThread(function()
    while true do
        ...
        ESX.ShowAdvancedNotification("FlashSide", "~r~Levels", "Vous avez gagner ~r~10000~s~ XP!\nVous êtes maintenant libre, vous pouvez tout faire ^^","CHAR_SEALIFE", 7)
    end
end)
			...
			for i = 1, #disabledSafeZonesKeys, 1 do
				...
				if IsDisabledControlJustPressed(disabledSafeZonesKeys[i].group, disabledSafeZonesKeys[i].key) then
					...
					if disabledSafeZonesKeys[i].message then
						ESX.ShowAdvancedNotification("FlashSide", "~r~Sécurité", disabledSafeZonesKeys[i].message,"CHAR_SEALIFE", 7)
					end
				end
			end
		end
