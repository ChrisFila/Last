cfg_door = {}
cfg_door.Locale = 'fr'

cfg_door.DoorList = {
	---Main Floor
	{
		objName = `gabz_mrpd_reception_entrancedoor`, 
		objCoords  = vector3(434.74, -980.75, 30.81),
		textCoords = vector3(435.65, -981.81, 31.01),  
		job = 'police',
		locked = false, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_reception_entrancedoor`, 
		objCoords  = vector3(434.74, -983.07, 30.81),
		textCoords = vector3(435.65, -981.81, 31.01),  
		job = 'police', 
		locked = false, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_04`, 
		objCoords  = vector3(440.5201, -977.6011, 30.82319),
		textCoords = vector3(441.15, -977.73, 31.22319), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_05`, 
		objCoords  = vector3(440.5201, -986.2335, 30.82319),
		textCoords = vector3(441.17, -986.11, 31.22319), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	},

	{
		objName = `gabz_mrpd_door_01`, 
		objCoords  = vector3(438.1971, -996.3167, 30.82319),
		textCoords = vector3(438.12, -995.01, 31.22319),  
		job = 'police', 
		locked = true,
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_01`, 
		objCoords  = vector3(438.1971, -993.9113, 30.82319),
		textCoords = vector3(438.12, -995.01, 31.22319), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_reception_entrancedoor`, 
		objCoords  = vector3(440.7392, -998.7462, 30.8153),
		textCoords = vector3(441.85, -998.60, 31.2153),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_reception_entrancedoor`, 
		objCoords  = vector3(443.0618, -998.7462, 30.8153),
		textCoords = vector3(441.85, -998.60, 31.2153),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_05`, 
		objCoords  = vector3(452.2663, -995.5254, 30.82319),
		textCoords = vector3(452.75, -995.94, 31.22319),  
		job = 'police',
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_02`, 
		objCoords  = vector3(458.0894, -995.5247, 30.82319),
		textCoords = vector3(457.62, -996.10, 31.22319),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_05`, 
		objCoords  = vector3(458.6543, -990.6498, 30.82319),
		textCoords = vector3(458.552, -990.07, 31.22319),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_04`, 
		objCoords  = vector3(458.6543, -976.8864, 30.82319),
		textCoords = vector3(458.67, -977.56, 31.22319),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_reception_entrancedoor`, 
		objCoords  = vector3(455.8862, -972.2543, 30.81531),
		textCoords = vector3(457.09, -972.17, 31.21531), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_reception_entrancedoor`, 
		objCoords  = vector3(458.2087, -972.2543, 30.81531),
		textCoords = vector3(457.09, -972.17, 31.21531),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_01`, 
		objCoords  = vector3(469.4406, -985.0313, 30.82319),
		textCoords = vector3(469.34, -986.38, 31.22319),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_01`, 
		objCoords  = vector3(469.4406, -987.4377, 30.82319),
		textCoords = vector3(469.34, -986.38, 31.22319),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_02`, 
		objCoords  = vector3(472.9781, -984.3722, 30.82319),
		textCoords = vector3(474.25, -984.45, 31.22319), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_02`, 
		objCoords  = vector3(475.3837, -984.3722, 30.82319),
		textCoords = vector3(474.25, -984.45, 31.22319),    
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_04`,
		objCoords  = vector3(479.7534, -986.2151, 30.82319),
		textCoords = vector3(479.88, -987.44, 31.22319),   
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_05`, 
		objCoords  = vector3(479.7534, -988.6204, 30.82319),
		textCoords = vector3(479.88, -987.44, 31.22319),   
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_04`, 
		objCoords  = vector3(475.3837, -989.8247, 30.82319),
		textCoords = vector3(474.06, -989.81, 31.22319),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_05`, 
		objCoords  = vector3(472.9777, -989.8247, 30.82319),
		textCoords = vector3(474.06, -989.81, 31.22319), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_03`, 
		objCoords  = vector3(479.7507, -999.629, 30.78917),
		textCoords = vector3(479.70, -999.01, 31.18917), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_04`,
		objCoords  = vector3(476.7512, -999.6307, 30.82319),
		textCoords = vector3(476.81, -998.91, 31.22319),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	},

	{
		objName = `gabz_mrpd_door_03`, 
		objCoords  = vector3(486.110, -1002.430, 30.78697),
		textCoords = vector3(486.77, -1002.75, 31.18697),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_03`, 
		objCoords  = vector3(487.313, -1002.568, 30.78697),
		textCoords = vector3(486.77, -1002.75, 31.18697),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_03`, 
		objCoords  = vector3(486.802, -1000.518, 30.691),
		textCoords = vector3(486.840, -1000.722, 30.991),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5

	}, 

	{
		objName = `prop_gate_airport_01`, 
		objCoords  = vector3(410.886475, -1025.43127, 28.4585419),
		textCoords = vector3(411.75, -1021.62, 28.4585419), 
		job = 'police', 
		locked = true, 
		distance = 15, 
		size = 1.3,
	pickable = false
	}, 

	{
		objName = `prop_damdoor_01`, 
		objCoords  = vector3(735.98, 132.65, 80.72),
		textCoords = vector3(735.5, 133.1, 81.02), 
		job = 'police', 
		locked = true, 
		distance = 3.5,
		size = 1.3, 
		pickable = false

	}, 

	{
		objName = `gabz_mrpd_garage_door`, 
		objCoords  = vector3(431.4119, -1000.772, 26.69661),
		textCoords = vector3(431.4119, -1000.772, 27.09661),
		job = 'police', 
		locked = true, 
		distance = 10,
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_garage_door`, 
		objCoords  = vector3(452.3005, -1000.772, 26.69661),
		textCoords = vector3(452.3005, -1000.772, 27.09661), 
		job = 'police', 
		locked = true, 
		distance = 10, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_room13_parkingdoor`,  
		objCoords  = vector3(464.1566, -997.5093, 26.3707),
		textCoords = vector3(464.05, -975.25, 26.7707), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_room13_parkingdoor`, 
		objCoords  = vector3(464.1566, -997.5093, 26.3707),
		textCoords = vector3(464.05, -996.85, 26.7707),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_04`, 
		objCoords  = vector3(471.3753, -985.0319, 26.40548),
		textCoords = vector3(471.45, -986.25, 26.80548),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_05`, 
		objCoords  = vector3(471.3753, -987.4374, 26.40548),
		textCoords = vector3(471.45, -986.25, 26.80548),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_01`, 
		objCoords  = vector3(469.9274, -1000.544, 26.40548),
		textCoords = vector3(468.70, -1000.45, 26.80548),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_01`, 
		objCoords  = vector3(467.5222, -1000.544, 26.40548),
		textCoords = vector3(468.70, -1000.45, 26.80548),    
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_02`, 
		objCoords  = vector3(471.3679, -1007.793, 26.40548),
		textCoords = vector3(471.45, -1009.00, 26.80548),   
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_02`, 
		objCoords  = vector3(471.3758, -1010.198, 26.40548),
		textCoords = vector3(471.45, -1009.00, 26.80548),  
		job = 'police', 
		locked = false, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_03`, 
		objCoords  = vector3(469.7743, -1014.406, 26.48382),
		textCoords = vector3(468.55, -1014.50, 26.88382),   
		job = 'police', 
		locked = false, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_03`, 
		objCoords  = vector3(467.3686, -1014.406, 26.48382),
		textCoords = vector3(468.55, -1014.50, 26.88382),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_03`, 
		objCoords  = vector3(475.8323, -990.4839, 26.40548),
		textCoords = vector3(475.45, -990.11, 26.80548), 
		job = 'police', 
		locked = true, 
		distance = 2,
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_02`, 
		objCoords  = vector3(479.0624, -985.0323, 26.40548),
		textCoords = vector3(479.17, -986.27, 26.80548),
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_02`, 
		objCoords  = vector3(479.508, -987.530, 26.40548),
		textCoords = vector3(479.17, -986.27, 26.80548),
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 


	{
		objName = `gabz_mrpd_door_04`, 
		objCoords  = vector3(482.6694, -988.14, 26.40548),
		textCoords = vector3(482.55, -988.33, 26.80548),   
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_04`, 
		objCoords  = vector3(475.3837, -984.3722, 30.82319),
		textCoords = vector3(475.3837, -984.3722, 31.22319),  
		job = 'police', 
		locked = true, 
		distance = 2,
		size = 0.5
	},

																										

	{
		objName = `gabz_mrpd_door_04`, 
		objCoords  = vector3(482.6703, -995.7285, 26.40548),
		textCoords = vector3(482.64, -996.45, 26.40548),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_02`, 
		objCoords  = vector3(482.0686, -997.91, 26.4065),
		textCoords = vector3(480.85, -997.81, 26.8065),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_02`, 
		objCoords  = vector3(479.6638, -997.91, 26.4065),
		textCoords = vector3(480.85, -997.81, 26.8065), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5

	}, 

	{
		objName = `gabz_mrpd_door_01`, 
		objCoords  = vector3(479.06, -1003.173, 26.4065),
		textCoords = vector3(479.15, -1002.45, 26.8065), 
		job = 'police',
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_cells_door`, 
		objCoords  = vector3(481.0084, -1004.118, 26.48005),
		textCoords = vector3(481.70, -1004.11, 26.88005), 
		job = 'police', 
		locked = true, 
		distance = 1, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_cells_door`, 
		objCoords  = vector3(484.1764, -1007.734, 26.48005),
		textCoords = vector3(484.90, -1007.65, 26.88005),
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_cells_door`, 
		objCoords  = vector3(486.9131, -1012.189, 26.48005),
		textCoords = vector3(486.15, -1012.10, 26.88005),
		job = 'police', 
		locked = true, 
		distance = 2,
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_cells_door`, 
		objCoords  = vector3(483.9127, -1012.189, 26.48005),
		textCoords = vector3(483.25, -1012.25, 26.88005),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_cells_door`, 
		objCoords  = vector3(480.9128, -1012.189, 26.48005),
		textCoords = vector3(480.20, -1012.21, 26.88005), 
		job = 'police', 
		locked = true, 
		distance = 2,
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_cells_door`, 
		objCoords  = vector3(477.9126, -1012.189, 26.48005),
		textCoords = vector3(477.25, -1012.189, 26.88005),  
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_cells_door`, 
		objCoords  = vector3(476.6157, -1008.875, 26.48005),
		textCoords = vector3(476.55, -1008.15, 26.88005),  
		job = 'police', 
		locked = true, 
		distance = 2,
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_04`, 
		objCoords  = vector3(475.9539, -1010.819, 26.40639),
		textCoords = vector3(475.20, -1010.85, 26.80639),  
		job = 'police', 
		locked = true,
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_03`, 
		objCoords  = vector3(464.3086, -984.5284, 43.77124),
		textCoords = vector3(464.35, -983.80, 44.17124), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_04`, 
		objCoords  = vector3(459.9454, -981.0742, 35.10398),
		textCoords = vector3(459.15, -981.0742, 35.50398), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_05`, 
		objCoords  = vector3(459.9454, -990.7053, 35.10398),
		textCoords = vector3(459.30, -990.70, 35.50398), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_05`, 
		objCoords  = vector3(448.9868, -981.5785, 35.10376),
		textCoords = vector3(449.35, -982.15, 35.50376), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_04`, 
		objCoords  = vector3(448.9868, -990.2007, 35.10376),
		textCoords = vector3(449.60, -989.70, 35.50376), 
		job = 'police', 
		locked = true, 
		distance = 2, 
		size = 0.5
	}, 

	{
		objName = `gabz_mrpd_door_05`, 
		objCoords  = vector3(448.9846, -995.5264, 35.10376),
		textCoords = vector3(449.50, -995.90, 35.50376),
		job = 'police', 
		locked = true,
		distance = 2, 
		size = 0.5


	}, 

	{
		objName = `v_ilev_csr_door_r`,
		objCoords  = vector3(-60.54582, -1094.749, 26.88872),
		textCoords = vector3(-60.54582, -1094.749, 26.88872),
		locked = true,
		distance = 3.0,
		job = 'carshop'
	},
	{
		objName = `v_ilev_csr_door_l`,
		objCoords  = vector3(-59.89302, -1092.952, 26.88362),
		textCoords = vector3(-59.89302, -1092.952, 26.88362),
		locked = true,
		distance = 3.0,
		job = 'carshop'
	},
	{
		objName = `v_ilev_csr_door_l`,
		objCoords  = vector3(-39.13366, -1108.218, 26.7198),
		textCoords = vector3(-39.13366, -1108.218, 26.7198),
		locked = true,
		distance = 3.0,
		job = 'carshop'
	},
	{
		objName = `v_ilev_csr_door_r`,
		objCoords  = vector3(-37.33113, -1108.873, 26.7198),
		textCoords = vector3(-37.33113, -1108.873, 26.7198),
		locked = true,
		distance = 3.0,
		job = 'carshop'
	},
	{
		objName = `v_ilev_fib_door1`,
		objCoords  = vector3(-31.72353, -1101.846, 26.57225),
		textCoords = vector3(-31.72353, -1101.846, 26.57225),
		locked = true,
		distance = 3.0,
		job = 'carshop'
	},
    {
		objName = `v_ilev_fib_door1`,
		objCoords  = vector3(-33.80989, -1107.579, 26.57225),
		textCoords = vector3(-33.80989, -1107.579, 26.57225),
		locked = true,
		distance = 3.0,
		job = 'carshop'
	},

	{
		objName = `prop_strip_door_01`,
		objCoords  = vector3(127.9552, -1298.503, 29.41962),
		textCoords = vector3(127.9552, -1298.503, 29.41962),
		locked = false,
		distance = 3.0,
		job = 'unicorn'
	},
	{
		objName = `prop_magenta_door`,
		objCoords  = vector3(96.09197, -1284.854, 29.43878),
		textCoords = vector3(96.09197, -1284.854, 29.43878),
		locked = false,
		distance = 3.0,
		job = 'unicorn'
	},

 	{
		objName = `lr_prop_supermod_door_01`,
		objCoords  = vector3(-205.682, -1310.682, 30.297),
		textCoords = vector3(-205.682, -1310.682, 32.297),
		locked = true,
		distance = 15.0,
		job = 'mecano'
	}
}