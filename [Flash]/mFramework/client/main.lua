...
AddEventHandler('esx:addInventoryItem', function(item)
	ESX.UI.ShowInventoryIteiNotificationV3ication(true, item.label, item.count)
...
AddEventHandler('esx:removeInventoryItem', function(item, identifier)
	for i = 1, #ESX.PlayerData.inventory, 1 do
		if ESX.PlayerData.inventory[i].name == item.name and (not identifier or (item.unique and ESX.PlayerData.inventory[i].extra.identifier and ESX.PlayerData.inventory[i].extra.identifier == identifier)) then
			ESX.UI.ShowInventoryIteiNotificationV3ication(false, item.label, item.count)
...
AddEventHandler('esx:updateItemCount', function(add, itemName, count)
	for i = 1, #ESX.PlayerData.inventory, 1 do
		if ESX.PlayerData.inventory[i].name == itemName then
			ESX.UI.ShowInventoryIteiNotificationV3ication(add, ESX.PlayerData.inventory[i].label, add and (count - ESX.PlayerData.inventory[i].count) or (ESX.PlayerData.inventory[i].count - count))
...
AddEventHandler('esx:addWeapon', function(weaponName, weaponAmmo)
	...
	if not found then
		...
		ESX.UI.ShowInventoryIteiNotificationV3ication(true, weaponLabel, false)
		...
		if ESX.PlayerData.loadout[i].name == weaponName then
			...
			if component then
				...
				if not found then
					...
					ESX.UI.ShowInventoryIteiNotificationV3ication(true, component.label, false)
...
AddEventHandler('esx:removeWeapon', function(weaponName, ammo)
	for i = 1, #ESX.PlayerData.loadout, 1 do
		if ESX.PlayerData.loadout[i].name == weaponName then
			...
			ESX.UI.ShowInventoryIteiNotificationV3ication(false, weaponLabel, false)
			...
			if component then
				for j = 1, #ESX.PlayerData.loadout[i].components, 1 do
					if ESX.PlayerData.loadout[i].components[j] == weaponComponent then
						...
						ESX.UI.ShowInventoryIteiNotificationV3ication(false, component.label, false)
...
Citizen.CreateThread(function()
	while true do
		...
		if playerPed and playerPed ~= -1 then
			while GetResourceState('FlashSide') ~= 'started' do
				Citizen.Wait(10)
			end
