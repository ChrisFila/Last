...
--[[AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
	...
	for i = 1, 200 do
		Citizen.Wait(50)
		deferrals.update('Bienvenu(e) à toi ('..playerName..')🖖'..'\nRecherche de votre personnage sur FlashSide 🖐 ...('.. i ..'%)\nNous te souhaitons une très bonne aventure sur notre serveur 💙\nNotre Discord : discord.gg/fqzJ9MGw 📋 \nNotre Boutique : Soon... 🛒')
		Citizen.Wait(50)
	end
	Citizen.Wait(0)
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				sourceXPlayer.showAdvancedNotification("FlashSide", "~r~Inventaire", _U('gave_item', itemCount, ESX.Items[itemName].label, targetXPlayer.name), 'CHAR_SEALIFE', 7)
				targetXPlayer.showAdvancedNotification("FlashSide", "~r~Inventaire", _U('received_item', itemCount, ESX.Items[itemName].label, sourceXPlayer.name), 'CHAR_SEALIFE', 7)

				TriggerEvent("esx:giveitemalert", sourceXPlayer.name, targetXPlayer.name, itemName, itemCount)
			else
				sourceXPlayer.showAdvancedNotification("FlashSide", "~r~Inventaire", _U('ex_inv_lim', targetXPlayer.name), 'CHAR_SEALIFE', 7)
			end
		else
			sourceXPlayer.showAdvancedNotification("FlashSide", "~r~Inventaire", _U('imp_invalid_quantity'), 'CHAR_SEALIFE', 7)
		end
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			sourceXPlayer.showAdvancedNotification("FlashSide", "~r~Portefeuille", _U('gave_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, targetXPlayer.name), 'CHAR_SEALIFE', 9)
			targetXPlayer.showAdvancedNotification("FlashSide", "~r~Portefeuille", _U('received_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, sourceXPlayer.name), 'CHAR_SEALIFE', 9)

			TriggerEvent("esx:giveaccountalert", sourceXPlayer.name, targetXPlayer.name, itemName, itemCount)
		else
			sourceXPlayer.showAdvancedNotification("FlashSide", "~r~Portefeuille", _U('imp_invalid_amount'), 'CHAR_SEALIFE', 9)
		end
	elseif type == 'item_weapon' then
		...
		if sourceXPlayer.hasWeapon(itemName) then
			...
			if not targetXPlayer.hasWeapon(itemName) then
				...
				if itemCount > 0 then
					sourceXPlayer.showAdvancedNotification("FlashSide", "~r~Armes", _U('gave_weapon_withammo', weaponLabel, itemCount, targetXPlayer.name), 'CHAR_SEALIFE', 7)
					targetXPlayer.showAdvancedNotification("FlashSide", "~r~Armes", _U('received_weapon_withammo', weaponLabel, itemCount, sourceXPlayer.name), 'CHAR_SEALIFE', 7)
				else
					sourceXPlayer.showAdvancedNotification("FlashSide", "~r~Armes", _U('gave_weapon', weaponLabel, targetXPlayer.name), 'CHAR_SEALIFE', 7)
					targetXPlayer.showAdvancedNotification("FlashSide", "~r~Armes", _U('received_weapon', weaponLabel, sourceXPlayer.name), 'CHAR_SEALIFE', 7)

					TriggerEvent("esx:giveweaponalert", sourceXPlayer.name, targetXPlayer.name, itemName)
				end
			else
				sourceXPlayer.showAdvancedNotification("FlashSide", "~r~Armes", _U('gave_weapon_hasalready', targetXPlayer.name, weaponLabel), 'CHAR_SEALIFE', 7)
				targetXPlayer.showAdvancedNotification("FlashSide", "~r~Armes", _U('received_weapon_hasalready', sourceXPlayer.name, weaponLabel), 'CHAR_SEALIFE', 7)
			end
		end
...
AddEventHandler('esx:dropInventoryItem', function(type, itemName, itemCount)
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			xPlayer.showAdvancedNotification("FlashSide", "~r~Inventaire", _U('imp_invalid_quantity'), 'CHAR_SEALIFE', 7)
		else
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				--local pickupLabel = ('~r~%s~s~ [~r~%s~s~]'):format(ESX.Items[itemName].label, itemCount)
				--ESX.CreatePickup('item_standard', itemName, itemCount, pickupLabel, _source)
				xPlayer.showAdvancedNotification("FlashSide", "~r~Inventaire", _U('threw_standard', itemCount, ESX.Items[itemName].label), 'CHAR_SEALIFE', 7)
			end
		end
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			xPlayer.showAdvancedNotification("FlashSide", "~r~Portefeuille", _U('imp_invalid_amount'), 'CHAR_SEALIFE', 9)
		else
			...
			if (itemCount > account.money or account.money < 1) then
				...
				--local pickupLabel = ('~r~%s~s~ [~r~%s~s~]'):format(accountLabel, _U('locale_currency', ESX.Math.GroupDigits(itemCount)))
				--ESX.CreatePickup('item_account', itemName, itemCount, pickupLabel, _source)
				xPlayer.showAdvancedNotification("FlashSide", "~r~Portefeuille", _U('threw_account', ESX.Math.GroupDigits(itemCount), string.lower(accountLabel)), 'CHAR_SEALIFE', 9)
			end
		end
	elseif type == 'item_weapon' then
		itemName = string.upper(itemName)
		...
		if xPlayer.hasWeapon(itemName) then
			...
			--local pickupLabel = ('~r~%s~s~ [~r~%s~s~]'):format(weapon.label, weapon.ammo)
			--ESX.CreatePickup('item_weapon', itemName, weapon.ammo, pickupLabel, _source, weapon.components)

			if weapon.ammo > 0 then
				xPlayer.showAdvancedNotification("FlashSide", "~r~Armes", _U('threw_weapon_ammo', weapon.label, weapon.ammo), 'CHAR_SEALIFE', 7)
			else
				xPlayer.showAdvancedNotification("FlashSide", "~r~Armes", _U('threw_weapon', weapon.label), 'CHAR_SEALIFE', 7)
			end
		end
	end
end)
...
AddEventHandler('esx:useItem', function(itemName)
	...
	if xItem then
		if xItem.count > 0 then
			...
			xPlayer.showAdvancedNotification("FlashSide", "~r~Inventaire", _U('act_imp'), 'CHAR_SEALIFE', 7)
		end
	else
