...
ConfigAutoEcole.CheckPoints = {

    {
        ...
        Action = function(playerPed, vehicle, setCurrentZoneType)
            DrawMissionText(("Allez vers le prochain passage! Vitesse limite: ~r~%s~s~ km/h"):format(ConfigAutoEcole.SpeedLimits['residence']), 5000)
        end
    },
...

    {
        ...
        Action = function(playerPed, vehicle, setCurrentZoneType)
            Citizen.CreateThread(function()
                DrawMissionText("Faite rapidement un ~r~stop~s~ pour le piéton qui ~r~traverse", 5000)
...

    {
        ...
        Action = function(playerPed, vehicle, setCurrentZoneType)
            ...
            Citizen.CreateThread(function()
                DrawMissionText(("Marquer rapidement un ~r~stop~s~ et regardez à votre ~r~gauche~s~. Vitesse limite: ~r~%s~s~ km/h"):format(ConfigAutoEcole.SpeedLimits['town']), 5000)
                PlaySound(-1, 'RACE_PLACED', 'HUD_AWARDS', false, 0, true)
                FreezeEntityPosition(vehicle, true)
                Citizen.Wait(6000)

                FreezeEntityPosition(vehicle, false)
                DrawMissionText("~g~Bien!~s~ prenez à ~r~droite~s~ et suivez votre file", 5000)
            end)
        end
    },
...

    {
        ...
        Action = function(playerPed, vehicle, setCurrentZoneType)
            ...
            DrawMissionText(("Il est temps d'aller sur la rocade! Vitesse limite: ~r~%s~s~ km/h"):format(ConfigAutoEcole.SpeedLimits['freeway']), 5000)
            PlaySound(-1, 'RACE_PLACED', 'HUD_AWARDS', false, 0, true)
        end
    },
...

    {
        ...
        Action = function(playerPed, vehicle, setCurrentZoneType)
            setCurrentZoneType('town')
            DrawMissionText(("Entrée en ville, attention à votre vitesse! Vitesse limite: ~r~%s~s~ km/h"):format(ConfigAutoEcole.SpeedLimits['town']), 5000)
        end
    },
