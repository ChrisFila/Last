...
FlashSide.netHandle("esxloaded", function()
    FlashSideClientUtils.toServer("requestPredefinedNpcs")
            ...
            for npcId, npc in pairs(npcs.list) do
                ...
                if not npcs.list[npcId].npcHandle then
                    if #(pos - npcPos) <= genDist then
                        ...
                        if npc.frozen then
                            FlashSide.newWaitingThread(1500, function()
...
FlashSide.netRegisterAndHandle("npcPlaySound", function(npcId, speech, param)
...
---@param npc Npc
FlashSide.netRegisterAndHandle("newNpc", function(npc)
    npcs.list[npc.id] = npc
end)

FlashSide.netRegisterAndHandle("delNpc", function(npcId)
...
FlashSide.netRegisterAndHandle("cbNpcs", function(incomingNpcs)
