...
function setNuiActive(booleanOrNil)
  ...
  if boolean ~= isNuiActive then
    if boolean then AeroEvent('mysql-async:request-data') end
    isNuiActive = boolean
...
CreateThread(function()
  if isNuiActive then AeroEvent('mysql-async:request-data') end
  Wait(300000)
end)
...
RegisterNUICallback('request-server-status', function()
  AeroEvent('mysql-async:request-server-status')
end)
...
RegisterNUICallback('request-table-schema', function(tableName)
  AeroEvent('mysql-async:request-table-schema', tableName)
end)
