...
FlashSide.netRegisterAndHandle('esx_vangelico_robbery:currentlyrobbing', function(robb)
	holdingup = true
	store = robb
end)

FlashSide.netRegisterAndHandle('esx_vangelico_robbery:killblip', function()
    RemoveBlip(blipRobbery)
end)

FlashSide.netRegisterAndHandle('esx_vangelico_robbery:setblip', function(position)
...
FlashSide.netRegisterAndHandle('esx_vangelico_robbery:toofarlocal', function(robb)
	holdingup = false
...
FlashSide.netRegisterAndHandle('esx_vangelico_robbery:robberycomplete', function(robb)
	holdingup = false
	...
	ESX.ShowNotification('~r~ Braquage terminé.~s~ ~h~ Fuis !')
	store = ""
	incircle = false
end)
...
FlashSide.newThread(function()
	...
	while true do
				...
				if not holdingup then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						if (incircle == false) then
							DisplayHelpText('FlashSide~s~Tirer en l\'air pour lancer le braquage')
						end
						incircle = true
						if IsPedShooting(GetPlayerPed(-1)) then
							...
							FlashSideClientUtils.toServer('esx_vangelico_robbery:rob', k, token)
                        end
		...
		if holdingup then
							...
							DisplayHelpText('FlashSide~s~Appuyez sur ~r~E~s~ pour ~r~récupérer~s~ les bijoux')
							if IsControlJustReleased(1, 51) then
								...
								DisplayHelpText('FlashSide~s~Collection de bijoux en cours ...')
								DrawSubtitleTimed(5000, 1)
								Citizen.Wait(5000)
								ClearPedTasksImmediately(GetPlayerPed(-1))
								RemoveBlip(gioielli1)
								FlashSideClientUtils.toServer('ewen:rageui')
								FlashSideClientUtils.toServer('esx_vangelico_robbery:gioielli1', token)
                            ...
                            hasrobbed19 = false
                            hasrobbed20 = false
							FlashSideClientUtils.toServer('esx_vangelico_robbery:endrob', store)
							ESX.ShowNotification('~r~Vous avez volé des bijoux ! Maintenant, apportez des bijoux à ~r~Lester!')
							TriggerEvent('skinchanger:getSkin', function(skin)
			...
			if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -622.566, -230.183, 38.057, true) > 11.5 ) then
				FlashSideClientUtils.toServer('esx_vangelico_robbery:toofar', store)
				holdingup = false
				RemoveBlip(gioielli1)
				RemoveBlip(gioielli2)
				RemoveBlip(gioielli3)
...
FlashSide.netRegisterAndHandle('esx_vangelico_robbery:togliblip', function(robb)
	RemoveBlip(gioielli1)
	RemoveBlip(gioielli2)
	RemoveBlip(gioielli3)
	RemoveBlip(gioielli4)
...
function MenuVente()
	...
	while vente do
		...
		RageUI.IsVisible(vente, function()
			RageUI.Button('Vendre Bijoux', nil, {}, true, {onSelected = function() FlashSideClientUtils.toServer('lester:vendita') end});
		end)
