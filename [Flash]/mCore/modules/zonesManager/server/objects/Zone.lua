...
setmetatable(Zone, {
    __call = function(_, location, type, color, onInteract, helpText, drawDist, itrDist, restricted, baseAllowed)
        ...
        self.zoneID = (#FlashSideSZonesManager.list + 1)
        ...
        FlashSideSZonesManager.list[self.zoneID] = self
        return self
    end
})
...
function Zone:interact(source)
    ...
    FlashSideServerUtils.trace(("%s a intéragit avec la zone n°%s"):format(GetPlayerName(source),self.zoneID), FlashSidePrefixes.zones)
end
