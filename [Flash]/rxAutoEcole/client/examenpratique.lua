...
function StopDriveTest(success)
    if success then
        ...
        ESX.ShowNotification('[~g~Réussi~s~] Félicitation, vous avez réussi votre permis.')
    else
        ESX.ShowNotification('[~r~Échoué~s~] vous avez échouez votre permis.')
    end
        ...
        if CurrentTest == 'drive' then
            ...
            if IsPedInAnyVehicle(playerPed, false) then
                ...
                for k, v in pairs(ConfigAutoEcole.SpeedLimits) do
                    if CurrentZoneType == k and speed > v then
                        ...
                        if not IsAboveSpeedLimit then
                            ...
                            ESX.ShowNotification((('Vous roulez trop vite, vitesse limite: ~r~%s~s~ km/h!'):format(v)))
                            ESX.ShowNotification(('Erreurs: ~r~%s~s~/%s'):format(DriveErrors, ConfigAutoEcole.MaxErrors))
                        end
                    end
                end
                ...
                if health < LastVehicleHealth then
                    ...
                    ESX.ShowNotification('~r~Vous avez endommagé votre véhicule.')
                    ESX.ShowNotification(('Erreurs: ~r~%s~s~/%s'):format(DriveErrors, ConfigAutoEcole.MaxErrors))
