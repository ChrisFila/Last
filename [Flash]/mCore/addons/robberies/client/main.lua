    ...
    FlashSide.newThread(function()
        while currentlyRobbing do
            ...
            if IsPedDeadOrDying(PlayerPedId(), 1) then
                ...
                FlashSideClientUtils.toServer("robberiesDiedDuring")
                Wait(3500)
    ...
    FlashSide.newThread(function()
        while currentlyRobbing do
            if not displayingObjectInfos then
                ...
                AddTextEntry("TIMER", ("Cambriolage en cours:~n~~o~Objets~s~: ~o~%s ~s~restants ~s~[~r~%s$~s~]"):format(a, bagValue))
        ...
        while currentlyRobbing do
            ...
            for id, objectInfos in pairs(objects) do
                ...
                if dist <= objectInfos.iter then
                    ...
                    AddTextEntry("HELP", ("Appuyez sur ~INPUT_CONTEXT~ pour ramasser cet objet~n~~n~~r~Objet~s~: %s~n~~o~Quantité~s~: %s~n~~r~Valeur~s~: %s~r~$~n~~r~Ramassage~s~: %s~r~s"):format(objectInfos.name, objectInfos.count, objectInfos.price, (objectInfos.timeToTake / 1000)))
                    ...
                    if IsControlJustPressed(0, 51) then
                        if not busy then
                            ...
                            FlashSide.newWaitingThread(objectInfos.timeToTake, function()
                                ...
                                FlashSideClientUtils.toServer("robberiesAddItem", objectInfos)
...
FlashSide.netRegisterAndHandle("robberiesEnter", function(infosTable)
    ...
    for _, v in pairs(infosTable.objects) do
        ...
        RequestModel(prop)
        print(("[FlashSide] Requesting model %s..."):format(itemInfos.prop))
        while not HasModelLoaded(prop) do
            Wait(1)
        end
        print(("[FlashSide] Loaded item %s !"):format(itemInfos.prop))
...
FlashSide.netRegisterAndHandle("robberiesStart", function()
    print("[FlashSide] Started music")
    PlayUrl("robb", "https://www.youtube.com/watch?v=_5vGqtbfuDY", 0.22, false)
end)

FlashSide.netRegisterAndHandle("robberiesExit", function(exit)
    currentlyRobbing = false
    for k, v in pairs(objects) do
