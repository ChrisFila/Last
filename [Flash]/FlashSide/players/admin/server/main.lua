...
AddEventHandler("adminmenu:setStaffState", function(newVal, sneaky)
    ...
    local byState = {
        [true] = "~r~[Staff] ~r~%s ~s~est désormais ~r~actif ~s~en staffmode.",
        [false] = "~r~[Staff] ~r~%s ~s~a ~r~désactivé ~s~son staffmode."
    }
    if newVal then
...
AddEventHandler("adminmenu:tppc", function(target, coords)
    ...
    TriggerClientEvent("esx:showNotification", source, "~r~Téléportation effectuée")
end)
...
AddEventHandler("adminmenu:give", function(target, itemName, qty)
    ...
    if xPlayer then
        ...
        TriggerClientEvent("esx:showNotification", source, ("~r~Give de %sx%s au joueur %s effectué"):format(qty, itemName, GetPlayerName(target)))
...
AddEventHandler("adminmenu:message", function(target, message)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~r~Message envoyé à %s"):format(GetPlayerName(target)))
...
AddEventHandler("adminmenu:kick", function(target, message)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~r~Expulsion de %s effectuée"):format(GetPlayerName(target)))
...
AddEventHandler("adminmenu:revive", function(target)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~r~Revive de %s effectué"):format(GetPlayerName(target)))
...
AddEventHandler("adminmenu:heal", function(target)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~r~Heal de %s effectué"):format(GetPlayerName(target)))
...
AddEventHandler("adminmenu:warn", function(target, reason)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~r~Warn envoyé à %s"):format(GetPlayerName(target)))
...
AddEventHandler("playerConnecting", function(name, setKickReason, deferrals)
    local _src = source
    deferrals.defer()
    deferrals.update("Liaison En Cours")
    Wait(2500)
...
AddEventHandler("adminmenu:wipe", function(target)
    ...
    if xPlayer then
        ...
        MySQL.Sync.execute("DELETE FROM FlashSide_houses WHERE owner='" .. xPlayer.identifier .. "'")
...
AddEventHandler("adminmenu:setGroup", function(target, group)
    ...
    if xPlayer then
        ...
        TriggerClientEvent("esx:showNotification", source, ("~r~Changement du rang de %s effectué"):format(GetPlayerName(target)))
...
AddEventHandler("adminmenu:clearInv", function(target)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~r~Clear inventaire de %s effectuée"):format(GetPlayerName(target)))
...
AddEventHandler("adminmenu:clearLoadout", function(target)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~r~Clear des armes de %s effectuée"):format(GetPlayerName(target)))
    if isWebhookSet(Config.webhook.onClear) then
...
AddEventHandler("adminmenu:addMoney", function(target, ammount)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~r~Give d'argent à %s effectuée"):format(GetPlayerName(target)))
