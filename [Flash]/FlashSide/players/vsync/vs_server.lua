...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                --TriggerClientEvent('vSync:notify', source, 'Time is now ~r~frozen~s~.')
            else
                --TriggerClientEvent('vSync:notify', source, 'Time is ~r~no longer frozen~s~.')
            end
        else
    ...
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if not DynamicWeather then
                ...
                --TriggerClientEvent('vSync:notify', source, 'Changements métérologiques dynamiques ~r~activé~s~.')
            end
        else
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Error: ^1Vous n\'etes pas autorisé a utiliser cette commande.')
        end
    ...
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            ...
            if args[1] == nil then
                ...
                if validWeatherType then
                    --TriggerClientEvent('vSync:notify', source, 'Weather will change to: ~r~' .. string.lower(args[1]) .. "~s~.")
                    CurrentWeather = string.upper(args[1])
                    newWeatherTimer = 30
                    TriggerEvent('vSync:requestSync')
                else
...
RegisterCommand('blackout', function(source)
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            blackout = not blackout
            if blackout then
                --TriggerClientEvent('vSync:notify', source, 'Blackout is now ~r~enabled~s~.')
            else
...
RegisterCommand('morning', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(9)
        --TriggerClientEvent('vSync:notify', source, 'Time set to ~r~morning~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('noon', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(12)
        --TriggerClientEvent('vSync:notify', source, 'Time set to ~r~noon~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('evening', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(18)
        --TriggerClientEvent('vSync:notify', source, 'Time set to ~r~evening~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('night', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(23)
        --TriggerClientEvent('vSync:notify', source, 'Time set to ~r~night~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
    ...
    elseif source ~= 0 then
        if isAllowedToChange(source) then
            if tonumber(args[1]) ~= nil and tonumber(args[2]) ~= nil then
                ...
                --TriggerClientEvent('vSync:notify', source, 'Time was changed to: ~r~' .. newtime .. "~s~!")
                TriggerEvent('vSync:requestSync')
            else
